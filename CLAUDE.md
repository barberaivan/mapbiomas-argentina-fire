...
| `collection-01/docs/09-statistics.md` | **stage 5 + publication + launch — every burned-area / burnable-area number we report, and everything after the assets exist.** **§4 is the split that defines the stage**: the **numerator** (burned area by month × year × ecoregión × LULC) comes from **the network's toolkit**, run with our 13-class ecoregion vector registered in it — with the two things to settle with Brazil first (**which asset their dataset reads**: the landed month-of-burn collection or the not-yet-exported `*_coverage` subproducts; and **which LULC year** it crosses); the **denominator** is **ours and constant** — the **mode over 1998–2024** of the col-3 burnable classes, reduced as `eco13·10 + burnable` on the pinned grid, 26 rows, Drive as the comahue account (§4.2–§4.6); and **§4.4 says what a constant denominator changes** (`%` = "of what is burnable most of the time", a pure fire signal, `>100 %` no longer impossible, and **no per-LULC-class `%` without one more export**). **§1.2 September is ECOREGION ONLY** — departamento/provincia are December. **§2** why their shape is fast (the crossing lives in the pixel value, territory painted never intersected, a `bounds()` rectangle, no `tileScale`, one task) + our three divergences. **§3** pin `crs` + `crsTransform`, **never `scale: 30`**. **§5 the territorial layers and three measured traps** — the **`_r` rasters are numbered differently from their own vectors** (alphabetical; six of thirteen ids disagree, so a raster-derived table decoded with the vector's dictionary reports Monte as Pampa), the Latin-1 damaged `Stats-Arg_*` names, and the **string `GEOCODE`** on the 16-class asset; plus **§5.4** the departamento packing deferred to December. **§6 burnable** as a col-3 legend decision. **§7** the verification gates, built around the join. **§8 the factsheet numbers** — and **§8.2 fire counts from the LOCAL polygons: **calendar** year and month of `date_median` (everything the factsheet reports is calendar-year), each fire counted ONCE in the region holding its CENTROID**; we do NOT use Looker Studio. **§9** the other five network tables — **Brazil is helping export the remaining subproducts, except the scar-size side, which is blocked on our manual scar ingest**. **§10–§12** public assets, Workspace catastro, launch track; **§13** questions for Brazil, **§14** decisions on record |
| `collection-01/docs/10-factsheet_design.md` | the **factsheet's content** (in Spanish) — what each slide says and why: the multi-region graphic conventions (stable colour per region, the little map as the legend, `all_regions` vs focal variants, palette ordered by latitude) and the four analyses (mean annual burned proportion; the time series + GAM trend, and its "veces el año típico" normalisation; the pirogram; the intra-annual shape normalised per region). Pairs with docs/09, which says where every number comes from |
| `collection-01/docs/11-validation.md` | step 11 — **validation design**: stratified random sample of pixels, Olofsson (2014)/Stehman (2014) design-based estimators, error-adjusted burned area with CIs. Everything is **fire-year** (1 May → 30 Apr), so our layer is rebuilt from the calendar month-of-burn collection and every external product is `filterDate`d to the same window. **§4 is the exact strata-raster recipe** — the S2 union of our map + MCD64A1 + **VNP64A1 (VIIRS burned area, in GEE from Mar 2012)** + FireCCI51 + FIRMS, aggregated with `max` (never `mode`) onto a coarse grid that is **our 30 m grid decimated ×16 (~480 m), not an independent 500 m grid**, dilated with a 3×3 square kernel, reprojected back and minus S1; population = whole country (`ARG-Political_Level_1-Pais`, 279.27 Mha). Strata land in the existing IC **`FIRE/VALIDATION/sampling_strata`**, one 2-band image per fire year (`stratum` 1/2/3 + `burned` = our map's own call, so the drawn points carry the map class and the confusion matrix comes straight out of the sample CSV), keyed by the mandatory `year` (= FIRE year) + `collection` (= 1) properties. Also §5 the frozen 30 k-per-stratum ordered sample lists, §6 what 100/stratum/year buys and how to extend, §7 response design + QC, §9 why `stehman2014()` not `olofsson()`. **Note the code still cites this as `docs/10 §…`** |
...
- **`data/` directory naming** (steps 04–06, all in the Insync store): `snic-rasters/` →
  ...
  (docs/09 §1.1). Two rules: it is an **object** (a fire is an
...
- **`2-Statistics/`** is the other half people forget: **`toolkit/v03/` is the stage-5 method**
  (docs/09 §2 — we re-implement its shape in Python, in `collection-01/statistics/`; we do **not**
  fork its JavaScript), the six `toDrive-area-*` scripts are the spec of what each network number is
  (docs/09 §9), and `1-Burned_area_products/` holds the comparison series against

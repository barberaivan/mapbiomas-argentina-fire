...
| `collection-01/docs/09-statistics.md` | **stage 5 + publication + launch — every burned-area / burnable-area number we report, how to build it, and everything after the assets exist.** The *method* is the network's **`2-Statistics/toolkit/v03/`**; the *code* is **ours, in Python**, in `collection-01/statistics/`, exporting to **Drive** (§1.1 the module + data layout; **§1.2 September is ECOREGION ONLY** — departamento/provincia are December). **§2 why their shape is fast** (the cross-tab lives in the pixel value; territory is one painted raster, never an intersected vector; the reduction geometry is a `bounds()` rectangle; no `tileScale`; all years in one task — and note computing the image on the fly is *fine*), plus the table of where we diverge. **§3** pin `crs` + `crsTransform`, **never `scale: 30`**. **§4 D1, the big table** — one **int32** code per pixel, `ecoregion·10⁴ + month·10² + col-3 LULC of the PREVIOUS year`, `unmask(0)` so one space-filling table carries numerator *and* denominator, the burned flag NOT encoded (`month == 0` is "did not burn"), one grouped sum, Drive as the **comahue** account, and the small-rectangle test that must run first. **§5 the territorial layer** — the `Stats-Arg_*` / `_3857` families and the two measured traps (**duplicate 16-class assets**, one Latin-1 damaged; **`GEOCODE` is a STRING** on the clean one, so cast before `paint`), the **measured 16 → 13 crosswalk** (exact; Esteros del Iberá → Chaco), and **§5.4 the packed `ecoregion16·100000 + GEOCODE`** cut deferred to December. **§6 burnable** as a col-3 legend decision — which is what **cancels the fixed modal-`veg_fire` layer and both `workflow/11-*.py` scripts**. **§7** the verification gates (incl. the network's ~1 % staging tolerance). **§8 the factsheet numbers** — each analysis to its rows, **and §8.2 fire counts from the LOCAL polygons: fire-year, month of `date_median`, each fire counted ONCE in the region holding its CENTROID** — and the note that we do NOT use Looker Studio. **§9** D2, the six network-spec tables (publication, blocked on 07c/07d). **§10–§12** public assets vs COGs, the Workspace catastro, the launch track. **§13** the question list for the Brazil team, **§14** the decisions on record |

...
| `collection-01/validation/docs/design.md` | **validation** (episodic — it consumes the map, it is not a stage of it): a stratified random sample of pixels, Olofsson (2014)/Stehman (2014) design-based estimators, error-adjusted burned area with CIs. Everything is **fire-year** (1 May → 30 Apr), so our layer is rebuilt from the calendar month-of-burn collection and every external product is `filterDate`d to the same window. **Read its status box first**: the design is Iván's, the implementation is Ramón Peña Agrest's, **the Python wins where they disagree** (the draw is unstratified and split locally — `stratifiedSample` per stratum OOMs on the 2 M-edge national frame — and the reserve is 5 k, not 30 k), and ⚠️ **the strata were built against the `_v1` map while the product is now `_v2`**, so the frozen `burned` band must be re-sampled at the frozen `col`/`row` before any confusion matrix. Then: **"Building the strata rasters"** is the exact recipe — the S2 union of our map + MCD64A1 + **VNP64A1** + FireCCI51 + FIRMS, aggregated with `max` (never `mode`) onto **our 30 m grid decimated ×16 (~480 m), not an independent 500 m grid**, dilated 3×3, reprojected back and minus S1; population = whole country (279.27 Mha); strata land in `FIRE/VALIDATION/sampling_strata`, one 2-band image per fire year keyed by the mandatory `year` (= FIRE year) + `collection` (= 1). Also the frozen ordered sample lists, what 100/stratum/year buys and how to extend, the response design, and why `stehman2014()` not `olofsson()`. The two GEE-JS appendices are now in `validation/docs/notes/` |

...
| `collection-01/docs/09-statistics.md` | **stages 5–6 + launch — every burned-area / burnable-area number we report, how to build it, and everything after the assets exist.** Stage 5 is the network's **`2-Statistics/toolkit/v03/`**, not a reducer of ours: **§2 why it is fast** (the cross-tab lives in the pixel value; territory is one painted raster, never an intersected vector; the reduction geometry is a `bounds()` rectangle; no `tileScale`; all years in one task — and note computing the image on the fly is *fine*). **§3** where our code lives — a **fork** of their `core/` in the `fuego` repo (theirs is read-only), whose only logic change is `scale` → **`crs` + `crsTransform`**, because every raster here stays on the one lattice. **§4 the territorial layer** — the guide's one-asset-unique-id spec, and the existing **`Stats-Arg_*` family** (528 departamentos carrying their province, 24 provincias, 16 ecorregiones, vector + raster) painted and **packed** as `ecoregion16·100000 + GEOCODE` — packing instead of intersecting is what removes the sliver problem; includes the **measured 16 → 13 ecoregion crosswalk** (exact; Esteros del Iberá → Chaco), why we paint the vectors rather than read the EPSG:3857 rasters, the `paint` masking trap, and the Latin-1 name-encoding damage in those assets. **§5 the two dataset families** — **D1**, `month·100 + col-3 LULC of the PREVIOUS year` with `unmask(0)` so one space-filling table carries numerator *and* denominator; **D2**, the six network-spec tables off the published products with their same-year encoding, and why the two cannot agree. **§6 burnable** as a col-3 legend decision — which is what **cancels the fixed modal-`veg_fire` layer and both `workflow/11-*.py` scripts**. **§8** the verification gates (incl. the network's ~1 % staging tolerance), **§9** how each factsheet number falls out of D1 — **and the note that we do NOT use Looker Studio: Argentina's analysis is ours, in R, off these CSVs**. **§11** public assets vs Cloud-Storage COGs (the platform reads **GEE assets**), **§12** the Workspace subtheme/legend/territory catastro, **§13** the launch track (ATBD, methodology page, downloads, materials, event), **§16** the question list for the Brazil team |
| `collection-01/docs/10-validation.md` | step 10 — **validation design**: stratified random sample of pixels, Olofsson (2014)/Stehman (2014) design-based estimators, error-adjusted burned area with CIs. Everything is **fire-year** (1 May → 30 Apr), so our layer is rebuilt from the calendar month-of-burn collection and every external product is `filterDate`d to the same window. **§4 is the exact strata-raster recipe** — the S2 union of our map + MCD64A1 + **VNP64A1 (VIIRS burned area, in GEE from Mar 2012)** + FireCCI51 + FIRMS, aggregated with `max` (never `mode`) onto a coarse grid that is **our 30 m grid decimated ×16 (~480 m), not an independent 500 m grid**, dilated with a 3×3 square kernel, reprojected back and minus S1; population = whole country (`ARG-Political_Level_1-Pais`, 279.27 Mha). Strata land in the existing IC **`FIRE/VALIDATION/sampling_strata`**, one 2-band image per fire year (`stratum` 1/2/3 + `burned` = our map's own call, so the drawn points carry the map class and the confusion matrix comes straight out of the sample CSV), keyed by the mandatory `year` (= FIRE year) + `collection` (= 1) properties. Also §5 the frozen 30 k-per-stratum ordered sample lists, §6 what 100/stratum/year buys and how to extend, §7 response design + QC, §9 why `stehman2014()` not `olofsson()` |
...
- **`2-Statistics/`** is the other half people forget: **`toolkit/v03/` is what actually computes
  stage 5** (docs/09 §2 — we fork its `core/`), the six `toDrive-area-*` scripts are the older
  spec of what each number is (docs/09 §5.2), and `1-Burned_area_products/` holds the comparison
  series against MCD64A1/FireCCI/GABAM.

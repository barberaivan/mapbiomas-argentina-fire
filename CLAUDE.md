...
| `collection-01/docs/11-remap-and-factsheet.md` | **the September 2026 product update + factsheet production plan** (10 → 24 Sep, data to the designers ~16 Sep). §1 why the four `*_coverage` products must be re-exported regardless (they were built on a *preliminary* LULC col-3, `..._integration_v1_buffer`; the published one is `mapbiomas_argentina_collection3_pb`, verified on the same lattice, integer offset +67/−62 px). §2 the **agriculture over-mapping** problem, measured: `frac_agri >= 0.4` drops 54 k objects / 2.36 Mha (3.4 %), but leaves 1.37 Mha of cropland *pixels* in mixed objects, and 85 % of the affected area is **Chaco**, where land-clearing fire is partly real. §3 the dependency graph — what re-runs under which decision, and why the scar-size chain (07b/07c) is deferrable. §4 the parallelism ceiling (**~2 GEE tasks per account**, ≤3 accounts) and the `TESTS/` benchmark folder. §5 the **two analysis families** — pixel/area statistics (= the docs/09 stage-5 CSVs; burnable = **col-2 v8 `veg_fire`, previous year**) vs fire counts (local `sf` join, no GEE, threshold-agnostic). §6 the threshold explorer in the `fuego` repo. §8 the unresolved **Pampa problem**. §9 **what to ask the Brazil/platform team** + why `FINAL_PRODUCTS` → `mapbiomas-public` is a manual `copyAsset` they own |

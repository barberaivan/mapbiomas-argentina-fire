...
| `collection-01/docs/06-object_labels.md` | step 06, part 1 — **where the ~5 k labels come from**: collection in GEE on the step-04 `candseed` layer (the drawing-layer NAME is the metadata; one asset per collaborator), the objects-not-pixels join (`objects_labels_prep.R`, the R-tree block read, `terra::relate` never `sf::st_intersects`, never `st_cast` to POINT), and `clean_tagged()`'s cuts down to **5255 objects, 20 predictors**. Opens with the fact that governs every number step 06 produces: **the labelled sample is not a random sample of objects** |
| `collection-01/docs/06-object_model.md` | step 06, part 2 — **the classifier**. `Foundations`: why a model and not collection-00's empirical filter (0.62 / 0.50 on our labels), why probit BART, and why the labelled sample is the binding limitation. Then the `stochtree` fit and the chunked-prediction memory shape; the 20 predictors incl. 5 aggregated vegetation fractions; **"Why no predictor may identify the year"** — the `fire_year`/`year_calendar` prevalence leak and the `n_mean` era proxy, plus the two metrics collection 2 should stop computing; **"The three call columns"** (`fire_model` / `fire_tag` / deployed `fire`, and why `-1` not `NA`); **"The classification threshold"** — the out-of-fold Youden's J cut that RISES with size (0.20 / 0.44 / 0.69, `config/object_model_thresholds.csv`); **"Cross-validation"** and why the fold design decides the answer; importance + ALE; the population's uncertainty and the 1 ha cost/benefit; and **"Upload to GEE"** — all 28 fire-years, whole object set, zipped Shapefiles, the by-hand ingest and the validation gate. Its `Gotchas` carry two live defects of that ingest: **`objects_raw_2021` STILL hands consumers 1 249 duplicated features that no metadata count reveals — and the surplus is query-dependent** (re-measured 2026-09-18), and **`oid` is unique per OBJECT, not per row** (FY2000 is 4 rows) |
| `collection-01/docs/06-object_inspection.md` | step 06 — the **QGIS review layer** (a diagnostic tool, not a step, like `02-diagnostic_plots.md`): 28 GeoPackages joining predictions + metrics + the c-00 verdict onto the geometry already on disk, no GEE upload. The 34 curated fields, the four that are computed at export (`p_thresh`/`p_margin`/`th_band`/`verdict`), and **"Where to look first"** — the model and the old filter disagree on 26.6 % of object area, and `verdict = 'c00 only' AND area_ha >= 300` is the 7.5 % of area worth walking object by object |
| `collection-01/docs/07-vector_to_raster.md` | step 07, part 1 — **how the burned pixels are made**, sub-steps **07a–07c**, with the order-of-operations table and run commands for the whole step at the top. **"Object exclusion ruleset"** is the heart of it — the positive selection is `fire == 1 & area_ha >= 1 & not(A) & not(B)`, where **B** drops `frac_agri > T` (agriculture, everywhere) and **A** drops `frac_c15 > T` *within a 1 Jul–15 Nov window*, **confined** to a hand-drawn agricultural-Pampa AOI and to objects < 150 ha (unconfined it deleted two thirds of the Delta's burned area); why the fix is at the object level and not a raster mask, the three application points that must carry identical thresholds, and the `date_med`/`date_median` split. Then **"The `_v2` re-export"** — one constant (`C.PRODUCT_VERSION = 2`) renames the month collection, the nine subproducts, the scar rasters, the scar vectors and the polygon layer, and why versioning beats overwriting; the ONE pinned grid (`crsTransform`, never `scale=30`) and the col-0 lattice collision; the verified calendar-year partition (and FY1998's unpublishable Nov–Dec tail); the **`candseed==3` parent-date substitution** (881 k px / ~79 kha, and why it protects the scar product); why the LULC mask + solitary-pixel filter are **already embedded upstream** (verified: zero candidates on non-burnable `veg_fire`); the proof that polygon paint/rasterize reproduces the object pixel set **exactly**, and why `terra::cells()` is the right membership tool; then 07a the GEE month-of-burn build, 07b the local two-pass scar build, 07c the scar rasters and the mask invariant |
| `collection-01/docs/07-published_products.md` | step 07, part 2 — **what is packaged from those pixels**. "Products, and the shape they take": one asset per subproduct with one **band** per year, never one asset per year, and the size-range bands really are named `scar_area_ha_<year>`. **07d — the nine derived subproducts**: the four settled answers (which LULC — `C.PRODUCT_LULC`, the *published* col-3, deliberately a second constant from the frozen model-side `C.MAPBIOMAS_LULC`, all three sharing one byte-identical lattice; same year not previous; no region split; the shape), the nine encodings copied verbatim from the reference, the **four traps** in that reference code (`year_last_fire` bands are `classification_<year+1>` — an off-by-one the platform expects), what was verified on the landed assets, the three plumbing departures, and **"Namespace the task descriptions"** (`arg07d_`, because `listOperations()` is project-scoped across the whole network). **07e — the fire-object polygon layer for early users**: `burned_area_polygons_v2`, 1,012,645 objects / 63.33 Mha, the ten properties, ISO dates + `system:time_start` (and why no `time_end`), why `polygons` not `vectors`, which ACCOUNT submits it. Its two live storage defects: **`objects_raw_2021` STILL hands consumers 1 249 duplicated features that no metadata count reveals — and the surplus is query-dependent**, and **`oid` is unique per OBJECT, not per row** (FY2000 is 4 rows) |
| `collection-01/docs/08-postprocessing.md` | step 08 — **Argentina's route through the network's spec** (~1.2 k words). `Foundations` is the table that matters: which reference stages Argentina already satisfies **upstream and more strictly** (the LULC mask — zero `candseed > 0` on non-burnable `veg_fire`, verified; the solitary-pixel filter — the `>= 1 ha` cut is ≈ 11 px) and which it satisfies **better** (month measured per pixel from `abs_date`, not inferred from min-NBR). Then the vectors-only route and what dating per pixel buys (annual / monthly / scar_size agree pixel-for-pixel; the cost is that a fire straddling 31 December becomes two scars); what Argentina delivers (**all six** subproducts, delivered 2026-07-30); and **"Open decisions"** — the four still live: the `COLLECTION-1` spelling, whether we may publish the fire-object vectors, `frequency_burned`'s band name (the one needing an IPAM ruling), and the territorial layer |
| `collection-01/docs/external/mapbiomas-fuego-reference.md` | **the network's shared post-processing, as we read it** — code we do **not** own (`mapbiomas-fire` @ `904fbdf`, pinned 2026-09-18, and `origin/master` was already 68 commits ahead then). The six launch stages and who owns each, their mapping method (Alencar et al. 2022) vs ours, the layout of the reference repo, and the reference chain script by script: asset topology, `1-Post_classifications`, the raster subproducts and the scar-size chain. §1 gives the `curl …/export/pdf` recipe for reading the *Guía del Proceso de Lanzamiento* slides as a PDF. **Not a to-do list for Argentina, and it goes stale silently**; what Argentina actually runs is in the two `docs/07` files |
...
4. `06-object_model.R` — **object-level fire/non-fire classification, replacing collection-00's
   ...
   also find the fires the model missed (docs/06 "Upload to GEE"). Labels are prepared by
   ...
   **Never give the model a predictor that names the year or proxies for it** — that leak has been
   found and fixed twice here; docs/06 "Why no predictor may identify the year" before touching
   `PREDICTORS`.
5. **Step 07 — fire-year objects → calendar-year products** (docs/07 for the pixels and 07a–07c,
   docs/07-published_products for 07d and 07e). Four scripts:
...
6. **Step 08 — post-processing to the network's common products.** After step 07 the work stops being
   ours: every MapBiomas Fuego country runs the *same* post-processing to publish the *same*
   subproducts, even though their mapping method differs from ours. **Do not innovate there.** Two
   files, and they are different objects: `docs/08-postprocessing.md` is **Argentina's route** (what
   we deliver, how dating per pixel changes the products, what is still undecided), and
   `docs/external/mapbiomas-fuego-reference.md` is **the network's spec**, our reading of a repo we
   do not own, pinned to a commit. Several of the reference's stages — the LULC mask, the
...
See `collection-01/README.md` for the directory map, each step doc's `Run`
...
- **The GEE compute project is SHARED with the whole network — never touch a task you did not
  ...
  disambiguate by asset path but exists only on FINISHED operations. See docs/07-published_products "Namespace the task descriptions".
...
`collection-01/docs/external/mapbiomas-fuego-reference.md` for a map of the repo and what each script does.

...
├── statistics/             # Step 09 — every factsheet number and figure (docs/09-statistics.md)
...
│   ├── lulc_change_export.py        # ANÁLISIS 6: el PAÍS ENTERO x estado de fuego x cobertura de Y-1 x Y+offset — con control; `--window`/`--lat-split` para la trayectoria patagónica (docs/09 §5.7)
...
| `statistics/` | step-09: the toolkit's burned-area CSVs, the burnable denominator, the fire tables, the `factsheet_*` tables (incl. the four `factsheet_change*` and the two `factsheet_bosques*` of análisis 6), the **three rasters** (`arg_burn_perc_480m_{mean,max}.tif`, `arg_last_fire_480m_mean.tif`) and `figures/` (PNG + PDF of every figure) | ~160 MB |
...
# el caso patagónico (docs/09 §5.7.3): corte latitudinal + cohorte FIJA (la ventana de
...
# ⚠️ SÓLO EL PRIMERO ALIMENTA EL LANZAMIENTO DE SEPTIEMBRE (docs/09 §5.0, docs/10)
quarto render collection-01/notebooks/factsheet.qmd            # análisis 1-5, ~5 min  <- SE PUBLICA

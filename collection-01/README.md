...
│   ├── burn_perc_export.py          # RÁSTER 1: % de los años con fuego, 480 m, para el mapa de apertura (statistics/docs/statistics.md §5.5); `--reducer max` da el mapa de conteos enteros (§5.5.1)
│   ├── last_fire_export.py          # RÁSTER 2: el año del último fuego, 480 m, la figura del análisis 2 (statistics/docs/statistics.md §5.6)
│   ├── lulc_change_export.py        # ANÁLISIS 6: el PAÍS ENTERO x estado de fuego x cobertura de Y-1 x Y+offset — con control; `--window`/`--lat-split` para la trayectoria patagónica (statistics/docs/statistics.md §5.7)
...
counts** are local. Design, gates and decisions: `statistics/docs/statistics.md`; what each graphic says:
`statistics/docs/factsheet-sep2026-spec.md`.

```bash
# the two denominators — GEE batch tasks. --test-rect first (seconds, catches every
# structural error); both use the same Córdoba rectangle and must report the same total.
$PYTHON collection-01/statistics/burnable_export.py  --test-rect
$PYTHON collection-01/statistics/burnable_export.py  --export      # ~2 min, 52 rows
$PYTHON collection-01/statistics/lulc_area_export.py --test-rect
...
# Dos ventanas: Y+1 (el análisis) e Y+3 (la prueba del artefacto de la cicatriz, statistics/docs/statistics.md §5.7.2).
...
# el caso patagónico (statistics/docs/statistics.md §5.7.3): corte latitudinal + cohorte FIJA (la ventana de
# exclusión clavada en 5, para que los cuatro lags miren los mismos píxeles y los mismos años)
for off in 1 3 4; do
  $PYTHON collection-01/statistics/lulc_change_export.py --export --offset $off --window 5 --lat-split -44
done
...
# the analysis pass: the seven factsheet_*.csv tables, the trend GAMs, every per-region scalar
Rscript collection-01/statistics/factsheet_tables.R

# the figures -> data/statistics/figures/, PNG + PDF (~50 s)
# ⚠️ SÓLO EL PRIMERO ALIMENTA EL LANZAMIENTO DE SEPTIEMBRE (statistics/docs/statistics.md §5.0, statistics/docs/factsheet-sep2026-spec.md)
quarto render collection-01/notebooks/factsheet_sep2026.qmd    # las 3 láminas, ~7 min <- EL ENTREGABLE
quarto render collection-01/notebooks/factsheet.qmd            # análisis 1-5, ~5 min  <- el banco del que elige
quarto render collection-01/notebooks/factsheet_veg.qmd        # análisis 6, ~4 min    <- exploratorio
quarto render collection-01/notebooks/factsheet_veg_short.qmd  # versión corta, ~15 s  <- exploratorio
...
| `factsheet_sep2026.qmd` | 📦 **EL ENTREGABLE** (statistics/docs/statistics.md §5.10). **El factsheet de septiembre, lámina por lámina**: qué figura va en cada una, con qué nombre de archivo, qué dice el epígrafe y de qué tabla sale. Dibuja **versiones limpias** de figuras que `factsheet.qmd` ya tiene, bajo una regla: **sin título, sin subtítulo y sin epígrafe adentro de la imagen**; queda sólo la leyenda con su título, los títulos de los ejes y, en series y pirogramas, el nombre de la región. 16 imágenes (PNG 300 dpi + PDF vectorial) y **un `figNN_datos.csv` por figura** (los dos mapas no llevan CSV) en `data/statistics/factsheet_sep2026_figures_and_tables/`, que es **la carpeta que se le comparte al diseñador**. Lámina 1: el mapa de años quemados y qué se quema a nivel nacional (torta y barra, nivel 2). Lámina 2: el mapa del año del último fuego y cinco series de **Mha quemadas por año** (país, Monte, Espinal, Delta, Bosques Patagónicos), más las cuatro regiones en una **grilla 2 × 2** (`fig05-06-07-08_`) con las que decrecen arriba y las que aumentan abajo. Lámina 3: cuatro pirogramas en **Mha por mes** (país, Delta, Chaco, Bosques Patagónicos). Decisiones propias: la unidad es Mha y no % (el GAM **no** se reajusta, el quemable es constante); los decimales del eje Y salen del paso entre marcas (`num_auto`, si no Bosques Patagónicos dice `0,00` cinco veces); **los colores de cobertura son los de MapBiomas**, unidos por CÓDIGO de clase desde `lulc_area_eco13.csv` (`MB_PALETTE`, copiado del `13-reclass` del equipo de LULC); en la lámina 2 **el color va sólo en la banda** (dos puntos de plasma: violeta si la tendencia sube, naranja si baja, en el sentido del mapa del último fuego) y el dato va en negro; la lámina 3 va en **rocket** con el GAM de siempre (10 bases, `gamma = 1`) y en dos formatos, Argentina sola y **las tres regiones apiladas en una columna** (`fig10-11-12_`, con patchwork y no `facet_wrap`, porque el segundo eje tiene un factor distinto por región); y las clases de cobertura se ordenan por **leñosidad decreciente**, con una tercera versión de la figura 2 en **dos anillos** (nivel 2 adentro, agrupado al 4 %, y nivel 1 afuera: el anillo externo es la suma del interno). Los dos mapas **no se redibujan** si su PNG ya está escrito y el `.tif` no cambió: el render pasa de ~7 min a menos de 1 | `statistics/factsheet_tables.R` |
| `factsheet.qmd` | ✅ **EL BANCO DEL QUE ELIGE `factsheet_sep2026.qmd`** (statistics/docs/statistics.md §5.0): todas las variantes, con título, subtítulo y epígrafe adentro, que es lo que hace falta para decidir y justo lo que no se entrega. **Step 09 — the factsheet figures.** Draws only; every number comes from `data/statistics/`. Análisis 1–5 of `statistics/docs/factsheet-sep2026-spec.md` (proporción quemada media; la serie + tendencia GAM, abierta por **el mapa del año del último fuego**; **el patrón intraanual entero — 3.1 el mapa del mes pico en paleta cíclica, 3.2 el pirograma de doble eje, 3.3 la forma intraanual comparada, 3.4 el pirograma normalizado con los totales en el panel**; la composición de lo quemado; qué % de cada clase se quema) — el análisis 6 vive en `factsheet_veg.qmd`, each as an `all_regions` panel and 12 focal variants where that applies, plus the map-as-legend (which opens the notebook, with a names-alongside variant). Análisis 4 and 5 draw both legend levels, nivel-2 native classes first. The three pixel maps share one convention: **el cero es blanco y es una clase aparte**, magma for the frequency pair, plasma for the years (`statistics/docs/factsheet-sep2026-spec.md §0.1`). `params$write_plots` (default `true`) writes every figure to `data/statistics/figures/` as PNG **and** PDF | `statistics/fire_counts.R` → `statistics/factsheet_tables.R` |
| `factsheet_veg.qmd` | 🔬 **EXPLORATORIO** — no va a septiembre; material para el lanzamiento de diciembre y el paper (statistics/docs/statistics.md §5.0). **Análisis 6 — cómo cambia la cobertura alrededor del fuego**, entero y aparte (statistics/docs/factsheet-sep2026-spec.md §6). Vive solo porque `factsheet.qmd` tarda minutos y esto son tres CSV chicos. **Con CONTROL**: qué cambia con fuego y sin fuego, país y regiones, Y+1 e Y+3, y el cociente `q`; lo mismo **condicionado a la clase de ORIGEN** (el marginal engaña: mezcla sistemas que no pueden convertirse); las transiciones en seis subsecciones navegables (matriz de q, Sankey y composición × nivel 1/2); y **§5 el caso patagónico** — corte latitudinal en −44 y cohorte fija, donde el bosque quemado que deja de ser bosque va de 49 % en Y+1 a 78 % en Y+5. Escribe `fig06_*` | `statistics/factsheet_tables.R` |
| `factsheet_veg_short.qmd` | 🔬 **EXPLORATORIO** — no va a septiembre; es el ensayo de cómo se diría en una lámina (statistics/docs/statistics.md §5.0). **Análisis 6 en tres frases y dos figuras** — la versión de slide (statistics/docs/statistics.md §5.8, statistics/docs/factsheet-sep2026-spec.md §6.5). Mismas tablas, nada recalculado, renderiza en ~10 s. Las tres frases nacionales, **cada porcentaje con su superficie**: de las **61,4 Mha quemadas (1999-2024, hectárea-año)** cambian de clase **12,5 Mha / 20 %** (nivel 2) y **9,1 Mha / 15 %** (nivel 1); el país cambia **10,4 Mha/año, el 4,1 %** de sus 252 Mha de superficie vegetal; y de ese cambio el **3,4 %** ocurre donde ardió, **3,6 veces** lo que le tocaría por superficie (0,9 %). Cinco figuras: dos Sankeys del cambio (nivel 2 y nivel 1), **dos Sankeys completos con la diagonal** (`keep_unchanged = TRUE` — salen planos, y eso ES el resultado: el 85 % de lo que arde sigue siendo lo que era) y la figura de las dos barras de denominador distinto. **§1.3 es la trampa del umbral**: el corte del Sankey es POR TRANSICIÓN, así que un destino alimentado por muchos flujos chicos se dibuja más flaco de lo que es — a bosque llegan las mismas 0,891 Mha en los dos niveles, pero en 2 bandas contra 37. De un Sankey con umbral **no** se lee área de llegada. **§4 es sólo bosques** (statistics/docs/statistics.md §5.9): hay **cuatro** números de bosque y no son intercambiables — la familia deja de ser bosque con **q = 6,4**, el **bosque cerrado con q = 14,0**, bosque → agropecuario 10,9. El promedio de la familia no describe a ninguna clase (**bosque inundable q = 1,0: el fuego no le hace nada**), así que un número de bosque en una lámina tiene que ser de una clase. Más el Sankey del bosque quemado con su permanencia y la tabla de las cuatro ventanas — **Y+4 e Y+5 nacionales ya están exportados** en los `_n44`. Escribe `fig06c_` —prefijo propio, un solo productor por figura— y **no** lleva el control ni `q` del análisis general: ésos están en `factsheet_veg.qmd` | `statistics/factsheet_tables.R` |

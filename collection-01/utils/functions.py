...
Shared GEE helpers used across MULTIPLE collection-01 workflow steps — the small
common core, not a per-step dumping ground. Step-specific machinery lives with its
step (e.g. the burn-probability / time-series-metrics code is in
workflow/03-bp_ts_metrics.py; the SNIC code is in workflow/04-snic.py).

Currently shared here:
  * Landsat loading + cloud mask + ETM→OLI harmonization (get_landsat)   — steps 01, 03
  * the 21 focal spectral features (add_indices)                          — steps 01, 03
  * MapBiomas class / mosaic band selection (get_mb_class_band, …)        — steps 01, 03
  * point-id assignment (assign_point_ids)                                — step 01 + scripts
  * previous-year veg_fire class image (veg_fire_image)                   — steps 03, 04 + scripts
...
def veg_fire_image(year):
    ...
    Shared by step 03 (burn-prob prediction) and step 04 (SNIC per-veg thresholds).
    """

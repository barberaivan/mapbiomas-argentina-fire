...
def _parse_feature(token):
    ...
    produced by ``F.add_indices`` (``BLUE_t`` → ``BLUE``).  Prev factors carry a
...
def burn_prob_collection(year, tile_id, terms=None, keep_indices=False):
    ...
    mb_mosaic = F.get_mb_mosaic_bands(
    ...
    col = F.get_landsat(tile_geom, dates["prev_start"], dates["next_end"])
    col = mosaic_by_date(col)                    # one image per unique date

    def _add_bp(img):
        img = F.add_indices(img)
...
def main(year=None, tile_id=None, project=None, overwrite=False, status=False,
    ...
    if status:
        bpts_status(year=year, target_col=target_col)
        return
    bpts(year=year, tile_id=tile_id, export=True, overwrite=overwrite,
